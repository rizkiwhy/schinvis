                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            @if (Auth::user()->role_id === 1)
                                <form action="{{ route('admin.transaksi.peminjaman.pengajuan.updatepeminjamanpribadi') }}"
                                    method="post" class="form-horizontal" id="form-edit-pengajuan">
                                @elseif (Auth::user()->role_id === 2)
                                    <form
                                        action="{{ route('user.transaksi.peminjaman.pengajuan.updatepeminjamanpribadi') }}"
                                        method="post" class="form-horizontal" id="form-edit-pengajuan">
                                    @elseif (Auth::user()->role_id === 3)
                                        <form
                                            action="{{ route('management.transaksi.peminjaman.pengajuan.updatepeminjamanpribadi') }}"
                                            method="post" class="form-horizontal" id="form-edit-pengajuan">
                            @endif
                            @csrf
                                    ...
                                    <div class="input-group">
                                        <select type="text" name="statuspengajuan_id" id="statuspengajuan_id"
                                            ...
                                            <option value="{{ $data['pengajuanBarang']->statuspengajuan_id }}" selected
                                                disabled>
                                                {{ $data['pengajuanBarang']->statusPengajuan->namapeminjaman }}
                                            </option>
                                        </select>
                                    </div>
                                </div>
